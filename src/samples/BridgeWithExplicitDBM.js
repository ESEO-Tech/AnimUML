...
//	=> no longer available
// https://www.scribd.com/document/429779518/10-BCS2213-Modelling-With-UPPAAL (starting from slide 27)
				...
				features: `
					...
					resetClock(clock) {
						...
						if(internalDBM + "" === "false") {
							...
							dbm = new __builtin__.DBM(nextClock);
							internalDBM = dbm.dbm;
							dbm.set(nextClock, 0);
							
							dbm.xMinusYRelatesToM(nextClock, 0, 60, false);
						} else {
							dbm = new __builtin__.DBM(nextClock, internalDBM);
						}
						dbm.set(clock, 0);
					}
					clockGE(clock, upperBound) : bool {
						const dbm = new __builtin__.DBM(nextClock, internalDBM);
...
/**/
		...
		watchExpressions: {
			dbm: "new __builtin__.DBM(__ROOT__dbm.nextClock, __ROOT__dbm.internalDBM)",
